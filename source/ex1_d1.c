...
int main(int argc, char **argv){
	...
	if(argc == 2){
		...
		// Part D (Reduction) =========================================================
		int s = 0x7FFFFFFF; // Maximum 4-byte positive integer
		
		timer[6] = omp_get_wtime();
		#pragma omp parallel shared(N, B, s) private(i) 
		{
			#pragma omp for schedule(static, 1) reduction(min:s) // Find min with reduction
			for(i = 0; i < N*N; i++){
				s = s > B[i] ? B[i] : s;
			}
		}
		...
		printf("usage: ex1_1 [Test Number]\n\n");
	}
	return 0;
}
